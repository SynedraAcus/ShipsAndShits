...
label start:
    ...
    $ player_deck.append(Card(u'З', 1, spendable = True, tooltip = u'Свежие портовые сплетни'))
                                                ...
                                                "Извиниться":
                                                    ...
                                                    menu:
                                                            ...
                                                            label the_very_start_lazlo_party:
                                                                ...
                                                                menu:
                                                                    "Устроить незапланированный праздник":
                                                                        ...
                                                                        if ret == u'FДеньги':
                                                                            nvl clear
                                                                            "Да, проспонсировать кутеж полностью вам не под силу, но уж вложить свою долю вы определенно можете. На собранные в шапку (Олли недовольно ворчит во сне и прикрывает отныне беззащитные от солнца глаза рукой) деньги снаряжается экспедиция до рынков. Пока она в пути, Лазло и те из балагуров, кто допущен до кухни, соображают нехитрую закуску, а все остальные сдвигают столы и швыряются подушками — просто так, от чувства праздника."
                                                                                    ...
                                                                                    label the_very_start_lazlo_fucktheoptions:
                                                                                        ...
                                                                                        menu:
                                                                                            ...
                                                                                            "Открыть глаза и обнаружить в своей постели Олли Загиба":
                                                                                                nvl clear
                                                                                                "Хорошо, когда реальность соответствует ожиданиям: Олли (который вчера и сам соответствовал очень многим вашим ожиданиям) действительно сопит рядом, укрывшись с головой от бьющего солнца. Все попытки его растолкать оканчиваются ничем." 
                                                                    ...
                                                                    "Устроить праздник с прицелом набрать еще и матросов" if "sailors" not in the_very_start_crew:
                                                                        ...
                                                                        if ret == u'SДеньги':
                                                                            ...
                                                                            "Экспедиция возвращается быстро, и на в спешке сдвинутых столах появляется целая батарея бутылок. Разговор к тому времени идёт полным ходом: молодые бездельники — лучшая аудитория, когда речь заходит о контрабанде, драках на мечах и неизвестных берегах. Остаётся только закрепить успех парой захватывающих историй, а затем невзначай выдвинуть своё предложение."
                                                        ...
                                                        "Объяснить дело и спросить о грузах" if "cargo" not in the_very_start_list:
                                                            "Вы вкратце пересказываете сегодняшний день, объясняете, как теперь видите своё будущее и осведомляетесь, не знает ли любезнейший Лазло чего-нибудь, чем можно выгодно набить трюмы. Лазло задумчиво открывает и закрывает рот, затем еще раз и еще — и наконец торжествующе шелкает пальцами."
...
label new_conflict:
    ...
    "Хотя вообще-то этот конфликт предназначен для того, чтобы тестировать поражение. Чтобы было не так обидно, истраченные в нём одноразовые карты будут вам возвращены по завершении боя."
    $ player_deck.append(Card(u'Д', 1, tooltip=u'Разменный пятак', spendable=True))
    "Вам только что была выдана одноразовая единица денег, чтобы вы сами могли в этом убедиться."
    $ init_conflict_table([Card(u'Д', 10), Card(u'Д', 10)], return_spent=True)
    "Запускаем конфликт"
    if ret[0] == u'S':
